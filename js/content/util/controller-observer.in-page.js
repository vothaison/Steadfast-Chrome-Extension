...
(function () {
    ...
    // Make sure the whole thing runs after Ember complete its setup
    poll(
        function () {
            return window.App && App.__container__.lookup('controller:application').currentRouteName;
        },
        function () {
            run();
        },
        function () {
            console.warn("WARNING: couldn't wait for window.App");
        },
        3000, 500
        );

    //console.log('svu.controller-observer.in-page.js is RUNNING')
    ...
    function run() {
        ...
        var html = '<input type="text" data-provide="typeahead" class="sf_controller_observer_input" autocomplete="off">' +
            ...
            '<button class="sf_controller_observer_button_go btn btn-default" title="get value of current selected property">Go</button>' +
            '';
        ...
        function updateControllerInformation() {
            ...
            controllerName = App.__container__.lookup('controller:application').currentRouteName;
            ...
            if (modelDef) {
                props.pushObjects(modelDef.fields.map(function(field) { return field.name; }));

                props.pushObjects(Object.keys(controllerInstance));
                ...
                props = props.filter(onlyUnique).sort();
                //console.log('New fields', props)
            }
        }
